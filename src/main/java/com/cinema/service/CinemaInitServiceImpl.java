...
import com.cinema.dao.CategoryRepository;
...
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(v->{
			Stream.of("MegaRama", "IMAX", "FOUNOUN", "CHAHRAZAD", "DAOULIZ").forEach(nameCinema -> {
				...
				cinema.setNombreSalles(5+(int)(Math.random()));
				cinema.setVille(v);
	...
	public void initFilms() {
		...
		Stream.of("Vikings", "Seigneur des anneaux", "Spider man", "IRON Man", "Yougi yo").forEach(f -> {
			Film film = new Film();
			film.setTitre(f);
